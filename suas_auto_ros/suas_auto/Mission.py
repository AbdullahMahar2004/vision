...
# ---------------------------------------------------------------------------
LAP_WAYPOINTS = [
    (31.3095769, 30.0659171),   # WP1 — start
    (31.3095343, 30.0660105),   # WP2
    (31.3094310, 30.0660476),   # WP3
    (31.3095769, 30.0659171),   # back to WP1 — closes the loop
]

# ---------------------------------------------------------------------------
# LAWNMOWER SEARCH AREA  (Phases 3-4) — outer corners of the search boundary
#
# CAUTION: the new boundary is a rotated quadrilateral (not an axis-aligned
# box like the old one). generate_lawnmower() below only takes BL/BR/TL and
# assumes a right-angled rectangle (it steps north from BL->TL and derives
# each row's east extent from BL->BR), so it will NOT accurately cover a
# rotated shape like this one — rows will drift off the true boundary.
# For this boundary, laps3.py's generate_zigzag() (which bilinearly
# interpolates between all 4 corners) is the correct tool. Corners here are
# assigned by which edges connect around the perimeter (N->W->S->E->N):
#   BL = W vertex, TL = N vertex, BR = S vertex, TR = E vertex
# ---------------------------------------------------------------------------
CORNER_BL = (31.3095769, 30.0659171)   # W vertex
CORNER_BR = (31.3094310, 30.0660476)   # S vertex
CORNER_TL = (31.3096358, 30.0660996)   # N vertex
CORNER_TR = (31.3095379, 30.0661717)   # E vertex

TARGET_ALT   = -4.0   # climb altitude, NED (negative = up) — from search_waypoints.txt alt=10
# Reference launch point (informational only — takeoff/RTL use the EKF's own
# local origin, so this isn't consumed by the code): lat 31.3104685, lon 30.0651584
