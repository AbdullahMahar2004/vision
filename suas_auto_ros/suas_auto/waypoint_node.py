...
class WaypointNode(Node):
    def __init__(self):
        ...
        self.waypoints = [
            (31.3105058, 30.0653382, -4.0),
            (31.3105611, 30.0654102, -4.0)
        ]
    ...
    def subscribe_vehicle_position(self, vehicle_local_position):
        ...
        self.current_x = vehicle_local_position.x
        self.current_y = vehicle_local_position.y
    ...
    def timer_callback(self):
        ...
        if self.phase == 0 and abs(self.current_z + 4.0) > 0.5:
            self.alt_reached_count = 0
            sp.position = [0.0, 0.0, -4.0]
            ...
            if self.waypoint_ned is None:
                target_lat, target_lon, target_alt = self.waypoints[self.waypoint_index]
                north_m = (target_lat - self.ref_lat) * 111139.0
                east_m = (target_lon - self.ref_lon) * 111139.0 * math.cos(math.radians(self.ref_lat))
                self.waypoint_ned = [north_m, east_m, target_alt]
            ...
            distance = math.sqrt(
                (self.current_x - self.waypoint_ned[0]) ** 2 +
                (self.current_y - self.waypoint_ned[1]) ** 2 +
                (self.current_z - self.waypoint_ned[2]) ** 2
            )
            ...
            self.get_logger().info('All waypoints reached — commanding RTL')
            self.publish_vehicle_command(VehicleCommand.VEHICLE_CMD_NAV_RETURN_TO_LAUNCH)
            self.phase = 3
        elif self.phase == 3:
            # RTL commanded — PX4 now owns the flight, stop sending offboard setpoints
            return
